...
data_folder = '/home/tehan/PycharmProjects/Shane-AO-Reduction/Results/'
# files = glob.glob(f'{data_folder}*.fits')
files = glob.glob(f'{data_folder}TOI_5671_final.fits')
for i in range(len(files)):
    ...
    ax2 = fig.add_axes([0.57, 0.55, 0.3, 0.3])
    # print(hdul[0].header)
    ax2.imshow(d, origin='lower')  # , cmap='RdGy_r', vmin=-1, vmax=1
    ax2.hlines(100, 445, 535, colors='w')
    ax2.text(490, 125, r"$3''$", ha='center', c='w')
    width = 256
